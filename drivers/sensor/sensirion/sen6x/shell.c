...
#define SEN6X_GET_DEVICE_BINDING(sh, arg)                                                          \
	({                                                                                         \
		const struct device *dev_ = shell_device_get_binding((arg));                       \
		if (dev_ == NULL || !sen6x_device_check(dev_)) {                                   \
...
#define SEN6X_GET_LONG(sh, arg, ret, name)                                                         \
	({                                                                                         \
		long value_ = shell_strtol((arg), 0, (ret));                                       \
		if ((*ret)) {                                                                      \
...
#define SEN6X_GET_ULL(sh, arg, ret, name)                                                          \
	({                                                                                         \
		unsigned long long value_ = shell_strtoull((arg), 0, (ret));                       \
		if ((*ret)) {                                                                      \
			shell_error((sh), "Invalid %s: %s", name, (arg));                          \
			return *(ret);                                                             \
		}                                                                                  \
...
{
	const struct device *dev = SEN6X_GET_DEVICE_BINDING(sh, argv[1]);
	const struct sen6x_firmware_version *firmware_version = sen6x_get_firmware_version(dev);
...
static int cmd_set_pressure(const struct shell *sh, size_t argc, char *argv[])
{
	int ret = 0;
	const struct device *dev = SEN6X_GET_DEVICE_BINDING(sh, argv[1]);
	uint16_t ambient_pressure = SEN6X_GET_ULL(sh, argv[2], &ret, "ambient_pressure");
...
static int cmd_set_altitude(const struct shell *sh, size_t argc, char *argv[])
{
	int ret = 0;
	const struct device *dev = SEN6X_GET_DEVICE_BINDING(sh, argv[1]);
	uint16_t sensor_altitude = SEN6X_GET_ULL(sh, argv[2], &ret, "sensor_altitude");
...
/* clang-format off */
SHELL_STATIC_SUBCMD_SET_CREATE(sub_sen6x,
	...
	SHELL_CMD_ARG(product_name, &dsub_device_name, SEN6X_PRODUCT_NAME_HELP, cmd_product_name, 2, 0),
	...
	SHELL_CMD_ARG(set_temp_offset, &dsub_device_name, SEN6X_SET_TEMP_OFFSET_HELP, cmd_set_temp_offset, 6, 0),
	SHELL_CMD_ARG(set_temp_accel, &dsub_device_name, SEN6X_SET_TEMP_ACCEL_HELP, cmd_set_temp_accel, 6, 0),
	SHELL_CMD_ARG(set_voc_tuning, &dsub_device_name, SEN6X_SET_VOC_TUNING_HELP, cmd_set_voc_tuning, 8, 0),
	SHELL_CMD_ARG(set_nox_tuning, &dsub_device_name, SEN6X_SET_NOX_TUNING_HELP, cmd_set_nox_tuning, 8, 0),
	SHELL_CMD_ARG(set_co2_calib, &dsub_device_name, SEN6X_SET_CO2_CALIB_HELP, cmd_set_co2_calib, 2, 0),
	SHELL_CMD_ARG(set_pressure, &dsub_device_name, SEN6X_SET_PRESSURE_HELP, cmd_set_pressure, 2, 0),
	SHELL_CMD_ARG(set_altitude, &dsub_device_name, SEN6X_SET_ALTITUDE_HELP, cmd_set_altitude, 2, 0),
	SHELL_CMD_ARG(clean_fan, &dsub_device_name, SEN6X_CLEAN_FAN_HELP, cmd_clean_fan, 2, 0),
	SHELL_CMD_ARG(activate_heater, &dsub_device_name, SEN6X_ACTIVATE_HEATER_HELP, cmd_activate_heater, 2, 0),
	SHELL_CMD_ARG(voc_state_save, &dsub_device_name, SEN6X_VOC_STATE_SAVE_HELP, cmd_voc_state_save, 2, 0),
	SHELL_CMD_ARG(voc_state_restore, &dsub_device_name, SEN6X_VOC_STATE_RESTORE_HELP, cmd_voc_state_restore, 2, 0),
