...
static int process_will_topic_update(struct mqtt_sn_client *client, int64_t *next_cycle) {
	...
	if (client->will_topic_update.last_attempt == 0) {
		next_attempt = 0;
	} else {
	...
	if (next_attempt <= now) {
		if (!client->will_topic_update.retries--) {
...
static int process_will_message_update(struct mqtt_sn_client *client, int64_t *next_cycle) {
	...
	if (client->will_message_update.last_attempt == 0) {
		next_attempt = 0;
	} else {
	...
	if (next_attempt <= now) {
		if (!client->will_message_update.retries--) {
		      ...
		      struct mqtt_sn_data *topic_name)
{
	...
	if (topic) {
		if (topic->state != MQTT_SN_TOPIC_STATE_REGISTERED || topic->type != MQTT_SN_TOPIC_TYPE_PREDEF) {
			return -EALREADY;
		}
	}
	else {
...
static void handle_willtopicresp(struct mqtt_sn_client *client, struct mqtt_sn_param_willtopicresp *p) {
	LOG_INF("Received WILLTOPICRESP");
