...
static int sen6x_get_channel_format(enum sensor_channel chan, bool *out_is_signed,
				    size_t *out_divisor, size_t *out_multiplier)
{
	bool is_signed;
	size_t divisor;
	size_t multiplier = 1;
				 ...
				 uint32_t *fit, uint16_t max_count, void *data_out_)
{
	...
	ret = sen6x_get_channel_format(chan_spec.chan_type, &is_signed, &divisor, &multiplier);
	if (ret != 0) {
		return ret;
	}
	...
	if (is_signed) {
		...
		data_out->readings[0].value =
			(((int64_t)value_signed) << (31 - data_out->shift)) * multiplier / divisor;
	} else {
		if (value == UINT16_MAX) {
			return -ENODATA;
		}
		...
		data_out->readings[0].value =
			(((int64_t)value) << (31 - data_out->shift)) * multiplier / divisor;
	}
