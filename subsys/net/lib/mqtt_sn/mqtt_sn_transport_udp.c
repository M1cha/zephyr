...
static int prepare_unicast_socket(struct mqtt_sn_transport_udp *udp, const struct net_sockaddr *dest_addr, size_t addrlen) {
	int ret;
	...
	udp->sock = zsock_socket(dest_addr->sa_family, SOCK_DGRAM, 0);
	if (udp->sock < 0) {
		return -errno;
	}
			 ...
			 const void *dest_addr, size_t addrlen)
{
	...
	if (dest_addr == NULL) {
		...
		rc = zsock_sendto(udp->sock, buf, sz, ZSOCK_MSG_DONTWAIT, &udp->addr,
				  udp->addrlen);
	} else {
