...
typedef struct sl_rail_ieee802154_address {
	union {
		RAIL_IEEE802154_Address_t legacy;
		struct {
			union {
...
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_address_t, short_address, shortAddress);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_address_t, long_address, longAddress);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_address_t, address_length, length);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_address_t, filter_mask, filterMask);

typedef struct sl_rail_ieee802154_config {
	union {
		RAIL_IEEE802154_Config_t legacy;
		struct {
...
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_config_t, p_addresses, addresses);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_config_t, ack_config, ackConfig);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_config_t, timings, timings);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_config_t, frames_mask, framesMask);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_config_t, promiscuous_mode, promiscuousMode);
ASSERT_FIELDS_COMPATIBLE(sl_rail_ieee802154_config_t, is_pan_coordinator, isPanCoordinator);
...
{
	return RAIL_IEEE802154_GetAddress(rail_handle, &p_address->legacy);
}
...
{
	return RAIL_IEEE802154_Init(rail_handle, &p_config->legacy);
}
