...
int sen6x_device_reset(const struct device *dev)
{
	...
	if (cfg->model == SEN6X_MODEL_SEN60) {
		return sen6x_reg_write_rtio(dev, REG_DEVICE_RESET_SEN60, NULL, 0, K_MSEC(1));
	} else {
		if (sen6x_is_measuring(dev)) {
			return -EPERM;
		}
		return sen6x_reg_write_rtio(dev, REG_DEVICE_RESET_SEN6X, NULL, 0, K_MSEC(1200));
	}
...
static void status_work_handler(struct k_work *work)
{
	...
	if (cfg->model == SEN6X_MODEL_SEN60) {
		...
		uint16_t status_sen60 = sys_get_be16(data->status_buffer);
	...
	if (data->status != status) {
		...
		data->status = status;
		LOG_DBG("status changed to 0x%08x", data->status);
...
static int sen6x_read_bytes(const struct device *dev, uint16_t reg, k_timeout_t delay, void *value_,
			    size_t value_len, size_t read_size)
{
	int ret;
...
int sen6x_get_product_name(const struct device *dev, char *name, size_t name_len)
{
	const struct sen6x_config *cfg = dev->config;
	...
	return sen6x_read_bytes(dev, REG_GET_PRODUCT_NAME, K_MSEC(20), name, name_len, 48);
}

int sen6x_get_serial_number(const struct device *dev, char *serial, size_t serial_len)
{
	const struct sen6x_config *cfg = dev->config;
	int ret;
	...
	if (cfg->model == SEN6X_MODEL_SEN60) {
		...
		ret = snprintf(serial, serial_len, "%02X%02X%02X%02X%02X%02X", serial_bytes[0],
			       serial_bytes[1], serial_bytes[2], serial_bytes[3], serial_bytes[4],
			       serial_bytes[5]);
		if (ret < 0 || (size_t)ret >= serial_len) {
			return -ENOBUFS;
		}
